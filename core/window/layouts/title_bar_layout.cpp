    ...
    switch (event.type) {
        case SDL_MOUSEBUTTONDOWN:
            if (event.button.button == SDL_BUTTON_LEFT) {
                // 增加调试日志
                DEARTS_LOG_INFO("SDL鼠标按下事件 - 坐标: (" + std::to_string(event.button.x) +
...
bool TitleBarLayout::isActuallyMaximized() const {
    ...
    if (SDL_GetDisplayUsableBounds(displayIndex, &usableBounds) == 0) {
        ...
        // 添加调试日志
        DEARTS_LOG_INFO("状态检测 - 当前位置: (" + std::to_string(currentPos.x) + "," + std::to_string(currentPos.y) +
                       ") 当前大小: " + std::to_string(currentSize.width) + "x" + std::to_string(currentSize.height) +
                       " 期望位置: (" + std::to_string(usableBounds.x) + "," + std::to_string(usableBounds.y) +
                       ") 期望大小: " + std::to_string(usableBounds.w) + "x" + std::to_string(usableBounds.h));

        // 主要检测：窗口大小是否匹配屏幕可用区域
        bool sizeMatches = (currentSize.width == usableBounds.w && currentSize.height == usableBounds.h);
        DEARTS_LOG_INFO("大小匹配: " + std::string(sizeMatches ? "是" : "否"));
        ...
        DEARTS_LOG_INFO("位置匹配(50px误差): " + std::string(positionMatches ? "是" : "否"));

        // 如果大小匹配且位置接近屏幕可用区域，认为窗口处于最大化状态
        if (sizeMatches && positionMatches) {
            DEARTS_LOG_INFO("状态检测结果: 最大化 (大小+位置都匹配)");
            return true;
        }

        // 另外，如果窗口大小匹配屏幕可用区域，也认为是最大化状态（即使位置有较大偏差）
        if (sizeMatches) {
            DEARTS_LOG_INFO("状态检测结果: 最大化 (仅大小匹配)");
            return true;
        }

        DEARTS_LOG_INFO("状态检测结果: 正常");
    } else {
        DEARTS_LOG_INFO("无法获取屏幕可用区域，状态检测结果: 正常");
    }
...
void TitleBarLayout::updateWindowState() {
    ...
    if (isMaximized_ != actuallyMaximized) {
        ...
        DEARTS_LOG_INFO("窗口状态已同步: " + std::string(isMaximized_ ? "最大化" : "正常"));
    ...
    if (isMaximized_ && (normalWidth_ == 0 || normalHeight_ == 0)) {
        ...
        auto size = parentWindow_->getSize();
        saveNormalState(pos.x, pos.y, size.width, size.height);
        DEARTS_LOG_INFO("最大化时保存窗口状态: 位置(" + std::to_string(pos.x) + "," + std::to_string(pos.y) +
...
// 使用原始SDL实现（无边框窗口模式）
    ...
    bool actuallyMaximized = isActuallyMaximized();
    DEARTS_LOG_INFO("最大化按钮点击 - 当前窗口状态: " + std::string(actuallyMaximized ? "已最大化" : "正常"));
    DEARTS_LOG_INFO("保存的正常状态 - 位置: (" + std::to_string(normalX_) + "," + std::to_string(normalY_) +
    ...
    if (actuallyMaximized) {
        ...
        DEARTS_LOG_INFO("还原前窗口状态 - 位置: (" + std::to_string(beforePos.x) + "," + std::to_string(beforePos.y) +
                       ") 大小: " + std::to_string(beforeSize.width) + "x" + std::to_string(beforeSize.height));

        DEARTS_LOG_INFO("调用 setPosition...");
        parentWindow_->setPosition(WindowPosition(normalX_, normalY_));

        DEARTS_LOG_INFO("调用 setSize...");
        parentWindow_->setSize(WindowSize(normalWidth_, normalHeight_));

        DEARTS_LOG_INFO("调用 restore...");
        parentWindow_->restore();
        ...
        DEARTS_LOG_INFO("还原后窗口状态 - 位置: (" + std::to_string(afterPos.x) + "," + std::to_string(afterPos.y) +
                       ") 大小: " + std::to_string(afterSize.width) + "x" + std::to_string(afterSize.height));

        DEARTS_LOG_INFO("窗口已还原到位置: (" + std::to_string(normalX_) + "," + std::to_string(normalY_) +
                       ") 大小: " + std::to_string(normalWidth_) + "x" + std::to_string(normalHeight_));
    } else {
        // 保存当前状态并最大化
        auto pos = parentWindow_->getPosition();
        ...
        // 强制更新状态，而不是手动设置
        // isMaximized_ = true; // 移除手动设置，让 updateWindowState() 处理
    }

    // 强制同步窗口状态
    updateWindowState();

    DEARTS_LOG_INFO("=== 最大化按钮处理完成 ===");
}
