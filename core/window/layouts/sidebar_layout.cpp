...
SidebarLayout::SidebarLayout()
    ...
    , isExpanded_(true)
    , isAnimating_(false)
    , currentWidth_(200.0f)
    , targetWidth_(200.0f)
    , sidebarWidth_(200.0f)
    , collapsedWidth_(48.0f)
    , animationDuration_(300.0f)
    , animationStartTime_(0.0f)
    , activeItemId_("")
    , backgroundColor_(0.15f, 0.15f, 0.15f, 1.0f)
    , itemNormalColor_(0.2f, 0.2f, 0.2f, 1.0f)
    , itemHoverColor_(0.3f, 0.3f, 0.3f, 1.0f)
    , itemActiveColor_(0.0f, 0.5f, 1.0f, 1.0f)
    , itemTextColor_(0.8f, 0.8f, 0.8f, 1.0f)
    , itemTextHoverColor_(1.0f, 1.0f, 1.0f, 1.0f) {
    currentWidth_ = sidebarWidth_;
    targetWidth_ = sidebarWidth_;
}
 ...
 * 切换侧边栏展开状态
 */
void SidebarLayout::toggleExpanded() {
...
void SidebarLayout::addItem(const SidebarItem& item) {
    ...
    auto it = std::find_if(items_.begin(), items_.end(), 
                          [&item](const SidebarItem& existingItem) {
...
void SidebarLayout::renderExpanded() {
    ...
    // 渲染收起按钮（使用VS Code图标字体）
    float buttonWidth = 24.0f;
    float buttonHeight = 24.0f;
    float buttonX = currentWidth_ - buttonWidth - 8.0f;
    float buttonY = 8.0f;
    ...
    std::string buttonLabel = DearTs::Core::Resource::ICON_VS_CHEVRON_LEFT;  // 使用VS Code左箭头图标
    ImGui::PushID("collapse_button");
...
void SidebarLayout::renderCollapsed() {
    ...
    // 渲染展开按钮（使用VS Code图标字体）
    float buttonWidth = 24.0f;
    float buttonHeight = 24.0f;
    float buttonX = (collapsedWidth_ - buttonWidth) / 2.0f;
    float buttonY = 8.0f;
    ...
    std::string buttonLabel = DearTs::Core::Resource::ICON_VS_CHEVRON_RIGHT;  // 使用VS Code右箭头图标
    ImGui::PushID("expand_button");
    if (ImGui::Button(buttonLabel.c_str(), ImVec2(buttonWidth, buttonHeight))) {
        toggleExpanded();
    }
    ImGui::PopID();
...
void SidebarLayout::renderItem(const SidebarItem& item, bool isExpanded) {
    ...
    if (item.id == "productivity") {
        ...
        // 创建树节点
        ImGuiTreeNodeFlags flags = ImGuiTreeNodeFlags_SpanAvailWidth | ImGuiTreeNodeFlags_FramePadding | ImGuiTreeNodeFlags_OpenOnArrow | ImGuiTreeNodeFlags_NoTreePushOnOpen;
        ...
        if (nodeOpen || item.isExpanded) {
            ...
            for (const auto& child : item.children) {
                ...
                // 显示字符图标（不使用图片）
                ImGui::SetCursorPos(ImVec2(cursorPos.x + 30.0f, cursorPos.y + (itemHeight - ImGui::GetTextLineHeight()) / 2.0f + 2.0f));
                ImGui::Text("%s", child.icon.c_str());
                
                // 绘制文本（移除问号帮助提示）
                ImGui::SameLine();
                ImGui::SetCursorPos(ImVec2(cursorPos.x + 50.0f, cursorPos.y + (itemHeight - ImGui::GetTextLineHeight()) / 2.0f + 2.0f));
        ...
        // 创建可选择项
        ImGui::SameLine();
        ImGui::SetCursorPos(ImVec2(cursorPos.x + (isExpanded ? 32.0f : (collapsedWidth_ + iconSize) / 2.0f), cursorPos.y));
        std::string selectableId = "##" + item.id;
        bool itemClicked = ImGui::Selectable(selectableId.c_str(), isSelected, flags, ImVec2(isExpanded ? (currentWidth_ - 40) : (collapsedWidth_ - 16), itemHeight));
        ...
        if (isExpanded) {
            ImGui::SameLine();
            ImGui::SetCursorPos(ImVec2(cursorPos.x + 40.0f, cursorPos.y + (itemHeight - ImGui::GetTextLineHeight()) / 2.0f));
            ImGui::Text("%s", item.text.c_str());
            
            // 如果是可展开项，显示展开/收起图标
            if (item.isExpandable) {
                ImGui::SetCursorPos(ImVec2(currentWidth_ - 20.0f, cursorPos.y + (itemHeight - ImGui::GetTextLineHeight()) / 2.0f));
                if (iconFont) {
                    iconFont->pushFont();
                }
                ImGui::Text("%s", item.isExpanded ? DearTs::Core::Resource::ICON_VS_CHEVRON_DOWN : DearTs::Core::Resource::ICON_VS_CHEVRON_RIGHT);
                if (iconFont) {
                    iconFont->popFont();
                }
            }
 ...
 * 计算动画插值（缓出立方）
 */
float SidebarLayout::easeOutCubic(float t) const {
