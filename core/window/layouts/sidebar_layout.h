  ...
  namespace Core {
    namespace Window {
      ...
      struct SidebarItem {
        ...
        SidebarItem(const std::string &id, const std::string &text, bool active = false,
                    const std::string &tooltip = "", const std::string &iconPath = "", bool expandable = false) :
            id(id), text(text), isActive(active), tooltip(tooltip), iconPath(iconPath), isExpandable(expandable),
            isExpanded(false) {}
      };
       ...
       * @brief 侧边栏项目点击回调函数类型
       */
      using SidebarItemClickCallback = std::function<void(const std::string& itemId)>;
