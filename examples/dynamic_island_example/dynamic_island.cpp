...
void DynamicIsland::Draw(ImDrawList* dl, float x, float y, float width) {
    for (auto& n : m_items) {
        ...
        // 标题行：图标 + 标题（粗体，无窗口依赖的 AddText 绘制）
        ImFont* title_font = ImGui::GetIO().Fonts->Fonts.Size > 2
                                 ? ImGui::GetIO().Fonts->Fonts[2]
                                 : ImGui::GetFont();
        const ImU32 title_col = ImGui::ColorConvertFloat4ToU32(ImVec4(1, 1, 1, alpha));
        const std::string title_text = std::string(kind_icon(n.kind)) + "  " + n.title;
        dl->AddText(title_font, 24.0f, // Fonts[2] 标题字号，见 main.cpp
                    ImVec2(pmin.x + 18, pmin.y + 4), title_col, title_text.c_str());

        // 正文（展开时最多两行，支持自动换行）
        if (n.expand > 0.05f && !n.body.empty()) {
            ImFont* body_font = ImGui::GetIO().Fonts->Fonts.Size > 1
                                    ? ImGui::GetIO().Fonts->Fonts[1]
                                    : ImGui::GetFont();
            const ImU32 body_col = ImGui::ColorConvertFloat4ToU32(
                ImVec4(0.78f, 0.78f, 0.85f, alpha));
            dl->AddText(body_font, 12.0f, // Fonts[1] 正文字号，见 main.cpp
                        ImVec2(pmin.x + 18, pmin.y + 30), body_col,
                        n.body.c_str(), nullptr, pmax.x - 14);
        }
